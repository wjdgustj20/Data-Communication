...
public class FileAppLayer implements BaseLayer {
    ...
    private ArrayList<Boolean> ackChk =new ArrayList<Boolean>();   //ack저장할 리스트
    ...
    public class _FAPP_HEADER { // File layer에서 사용되는 프레임 구조체
    ...
    private void setFragmentation(int type){ // 헤더의 단편화정보를 설정하는 메소드
       if(type == 0) { // 처음
    ...
    public void setFileMsgType(int type) { // 파일 정보인지, 파일 데이터인지 구분해주는 fapp_msg_type 값을 설정
        m_sHeader.fapp_msg_type = (byte) type;
    }

    public void setFileName(String fileName) { // 파일이름 설정자
    ...
    public int calcFileFullLength(byte[] input) { // 파일의 총 크기를 계산
        int fullLength = 0;
    ...
    public boolean fileInfoSend(byte[] input, int length) { // 파일 정보 전송 함수
    ...
    class fileTransfer implements Runnable{ // thread를 생성하며 인자로 전달할 객체 클래스
       public fileTransfer() {}
       
       @Override
       public void run() { // thread 실행시 수행할 메소드
          setAndStartSendFile();
       }
    }
    ...
    public void setAndStartSendFile() {
        ...
        try (FileInputStream fileInputStream = new FileInputStream(sendFile)) { // 파일스트림을 가져 옴
            ...
            BufferedInputStream fileReader = new BufferedInputStream(fileInputStream); // 파일스트림을 읽어 올 버퍼스트림
            ...
            ((ChatFileDlg)this.GetUpperLayer(0)).progressBar.setMaximum(sendTotalLength); // progressbar의 최댓값을 파일 전체 길이로 설정
            if(sendTotalLength <= 1448) { // 단편화 없이 전송 
                // 파일 정보 전송
                ...
                ((ChatFileDlg)this.GetUpperLayer(0)).progressBar.setValue(sendedLength); // progressbar 현재 진행값 업데이트
            } else { // 파일 길이가 1448보다 클때 단편화 필요
                sendedLength = 0;
                // 파일 정보 전송
                ...
                while(fileReader.read(sendData) != -1 && (sendedLength + 1448 < sendTotalLength)) { // 중간 데이터이면
    ...
    public void waitACK(){   //ACK체크 //ACK 들어올 때까지 대기 wait
       while(ackChk.size()<=0 ){
          try{
             Thread.sleep(10);
          }catch(InterruptedException e){
    ...
    public synchronized boolean Receive(byte[] input) { // 데이터를 수신 처리 함수
        ...
        if(checkReceiveFileInfo(input)) { // 파일의 정보를 받은 경우, 파일 이름과 길이 정보
            ...
            if (checkNoFragmentation(input)) {
                ...
                try(FileOutputStream fileOutputStream = new FileOutputStream(file)) { // 저장할 파일 스트림을 열어,
                    fileOutputStream.write(fileByteList.get(0)); // 받은 데이터를 작성
                } catch (IOException e) {
                    e.printStackTrace();
                }
            } else {
                ...
                if(checkLastDataFrame(input)) {
                    ...
                    if(sortFileList(lastFrameNumber)) { // 프레임을 모두 받았으면
                        try(FileOutputStream fileOutputStream = new FileOutputStream(file)) { // 저장할 파일 스트림을 열어,
                            for (int frameCount = 0; frameCount < (lastFrameNumber + 1); ++frameCount) { // 총 프레임 개수만큼 반복하며
                                data = RemoveCappHeader(fileSortList.get(frameCount), fileSortList.get(frameCount).length); // 헤더를 제거하고
        ...
        ((EthernetLayer)this.GetUnderLayer()).fileSend(null, 0);      //ack송신
        return true;
    }

    public void resetSeqNum() { // seqnum을 초기화하는 메소드
    ...
    public boolean Send(byte[] input, int length) { // 데이터 전송 함수
        byte[] bytes = this.ObjToByte(m_sHeader, input, length); // 헤더와 데이터를 합친 프레임을
        waitACK();
        ((EthernetLayer)this.GetUnderLayer()).fileSend(bytes, length + 12); // 하위 레이어로 전달
        return true;
    }

    private byte[] ObjToByte(_FAPP_HEADER m_sHeader, byte[] input, int length) { // 받은 헤더와 데이터를 바이트 프레임 배열에 넣어줌
        // header 길이 + 데이터 길이
    ...
    public boolean checkReceiveFileInfo(byte[] input) { // 받은 데이터가 파일 정보인지 확인
