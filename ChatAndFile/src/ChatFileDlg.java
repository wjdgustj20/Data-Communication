...
public class ChatFileDlg extends JFrame implements BaseLayer {
   ...
   public static void main(String[] args) {
      ...
      m_LayerMgr.ConnectLayers(" NI ( *Ethernet ( *ChatApp ( *GUI ) *FileApp ( *GUI ) )"); //채팅과 파일 레이어를 대등하게 연결
   }
   ...
   public ChatFileDlg(String pName) {
      ...
      filePanel.setBorder(new TitledBorder(UIManager.getBorder("TitledBorder.border"), "파일전송",
            TitledBorder.LEADING, TitledBorder.TOP, null, new Color(0, 0, 0)));
      filePanel.setBounds(10, 290, 360, 90);
      contentPane.add(filePanel);
      filePanel.setLayout(null);
      
      path = new JTextArea();
      path.setBounds(5, 20, 265, 25);
      filePanel.add(path); //경로 자동 입력되는 구간
      path.setLayout(null);
      
      File_select_Button = new JButton("파일...");
      File_select_Button.setBounds(280, 20, 70, 25);
      File_select_Button.addActionListener(new setAddressListener());
      filePanel.add(File_select_Button);// file select
      File_select_Button.setLayout(null);
      
      progressBar = new JProgressBar();
      progressBar.setBounds(5, 55, 265, 25);
      progressBar.setStringPainted(true); //퍼센트 나타냄
      ...
      File_send_Button.setBounds(280, 55, 70, 25);
      File_send_Button.addActionListener(new setAddressListener());
      File_send_Button.setEnabled(false); //파일 선택하기 전에는 전송 버튼 누를 수 없도록
      filePanel.add(File_send_Button);// file send
      File_send_Button.setLayout(null);
   ...
   class setAddressListener implements ActionListener {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         else if (e.getSource() == File_select_Button) { //파일 선택 버튼
            JFileChooser fileChooser = new JFileChooser();//파일 선택 객체 생성
            fileChooser.setCurrentDirectory(new File("C:\\")); //파일 창의 기본 경로를 C:\\에서 시작하도록
            int result = fileChooser.showOpenDialog(null); //창 띄우기
            if(result != JFileChooser.APPROVE_OPTION) {
               JOptionPane.showMessageDialog(null, "파일 선택 오류");
               return; //선택 없이 취소 눌렀을 경우
            }
            else { //파일 선택했을 경우경우
               object = fileChooser.getSelectedFile(); //대상 파일을 선택한 파일로 변경
               path.setText(fileChooser.getSelectedFile().getPath().toString()); //경로 텍스트 필드에 해당 파일의 경로를 문자열 변환하여 삽입
               File_send_Button.setEnabled(true); //파일 전송 버튼 클릭 가능하도록 변경
            }
         }
         else if (e.getSource() == File_send_Button) { //파일 전송 버튼 눌렀을 때
            if (Setting_Button.getText() == "Reset") { //주소 설정 완료
               try {
                  File_send_Button.setEnabled(false); //전송 버튼 클릭 후에는 파일 선택되기 전까지 불가능하도록 변경
                  
                  
                  ((FileAppLayer)m_LayerMgr.GetLayer("FileApp")).sendFile();
                  //파일창에 입력된 메시지를 FileApplayer로 보냄
                  //이때, 실시간 progressbar 변경 위해 생성한 메소드인 sendFile() 호출
